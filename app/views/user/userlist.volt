  ...
  {% endif %}
	...
	{% if page.items is defined %}
		{% for entity in page.items %}
			...
			{% endfor %}
			<td width ="2%">
			{% if special_permission['add_user_role']=='Y' %}
				{{link_to('userrole/list/'~entity.id,'<i class="fa fa-shield"></i>','class':'btn btn-icon-only blue')}}
      {% endif %}
			</td>
      <td width ="2%">
			{% if special_permission['change_password']=='Y' %}
			{{link_to('user/set_password_change/'~entity.id,'<i class="fa fa-lock"></i>','class':'btn btn-icon-only yellow')}}
      {% endif %}
			</td>
			<td width ="2%">

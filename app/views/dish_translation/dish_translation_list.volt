...
{% block content %}
  <!-- GRID SEARCH -->
	<div align="left" >
	{{ form(searchroute~'/' ~dishid, "method":"post", "autocomplete" : "off") }}
	<div class="row">
  ...
  <!-- END GRID SEARCH-->
	...
	{% if permissions['create']=='Y' %}
	 <!-- NEW ITEM ICON-->
	<div align="left">{{ link_to(newroute ~'/' ~dishid,'<i class="fa fa-plus"></i>','class':'btn btn-icon-only blue')}}</div>
  {% endif %}
	<br>
	{% if noitems ==""%}
	...
	{% if page.items is defined %}
		{% for entity in page.items %}
			...
			<td width ="2%">
				...
				{{link_to(editroute~entity.id~'/' ~dishid,'<i class="fa fa-edit"></i>','class':'btn btn-icon-only green')}}
				{% endif %}
			</td>
			<td width ="2%">
